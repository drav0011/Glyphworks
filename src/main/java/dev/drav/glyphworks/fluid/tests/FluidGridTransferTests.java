 ...
 *
 * <p>
 * All tests use a flat East-West layout (positions relative to origin):
 * 
 * <pre>
...
public final class FluidGridTransferTests {
    ...
    /**
     * Places {@code blockId} at the given coordinates and registers grid
     * connections.
     */
    ...
    /**
     * Returns the {@link FluidContainerComponent} for the grid block at
     * {@code pos},
     ...
     *
     * <p>
     * The source is OUTPUT-only (East face) and the sink is INPUT-only
    ...
    /**
     * Test 2: after a successful transfer the traversed pipes are locked to the
     * fluid type.
     *
     * <p>
     ...
     *
     * <p>
     * The inner relay pipe (at {@code ox+2}) is manually locked to
    ...
    /**
     * Test 4: no fluid is moved when the sink container is already at full
     * capacity.
     *
     * <p>
     * The sink (INPUT-only, capacity {@link #SINK_CAPACITY}) is pre-filled to
    ...
    private static TestCase noTransferWhenSinkIsFull() {
        return new TestCase("no_transfer_when_sink_is_full", 6, 3, 3)
                ...
                .step(Steps.run(ctx -> {
                    ...
                    FluidContainerComponent fccSink = getContainer(w, new Vector3i(ox + 3, oy, oz));
                    if (fccSink != null)
                        fccSink.fill(FLUID_ID, SINK_CAPACITY);
                    // Fill the source with the test amount.
                    FluidContainerComponent fccSrc = getContainer(w, new Vector3i(ox, oy, oz));
                    if (fccSrc != null)
                        fccSrc.fill(FLUID_ID, FILL_AMOUNT);
                }))
                .step(Steps.wait(WAIT_TICKS))
                .step(Steps.assertThat(ctx -> {
                    World w = ctx.getWorld();
